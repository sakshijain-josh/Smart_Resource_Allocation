...
RSpec.describe Booking, type: :model do
  ...
  describe "validations" do
    ...
    it "is invalid outside business hours" do
      # 7 AM
      date = Date.new(2027, 1, 6)
      start_time = Time.zone.local(date.year, date.month, date.day, 7, 0, 0)
      end_time = Time.zone.local(date.year, date.month, date.day, 8, 0, 0)
    ...
    it "is invalid on holidays" do
      ...
      start_time = Time.zone.local(holiday_date.year, holiday_date.month, holiday_date.day, 10, 0, 0)
      end_time = Time.zone.local(holiday_date.year, holiday_date.month, holiday_date.day, 11, 0, 0)

      travel_to Time.zone.local(2026, 12, 31, 10, 0, 0) do
        booking = build(:booking, start_time: start_time, end_time: end_time)
        expect(booking).not_to be_valid
    ...
    it "detects overlaps with approved bookings" do
      date = Date.new(2027, 1, 6)
      start1 = Time.zone.local(date.year, date.month, date.day, 10, 0, 0)
      end1 = Time.zone.local(date.year, date.month, date.day, 11, 0, 0)

      create(:booking, resource: resource, start_time: start1, end_time: end1, status: :approved)

      # Overlapping booking
      start2 = Time.zone.local(date.year, date.month, date.day, 10, 30, 0)
      end2 = Time.zone.local(date.year, date.month, date.day, 11, 30, 0)
